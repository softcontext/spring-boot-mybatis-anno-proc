...
CALL INSERT_BOOK('00001','NA',10000,'novel',@RESULT);
SELECT @RESULT;

CALL INSERT_BOOK('00002','AA',20000,'art',@RESULT);
SELECT @RESULT;

CALL INSERT_BOOK('00003','NB',30000,'novel',@RESULT);
SELECT @RESULT;

-----------------------------------------------------------------------
-- create procedure
-----------------------------------------------------------------------
--그런데 요구 사항이 발생되어 BOOKS테이블의 원가(bookOriginPrice)가 
--소설(novel)은 1000원이 올랐고, 
--art는 1500원, 
--그외는 2000원이 올랐다고 한다. 
--그렇다면 각각 테이블을 읽어들여 
--BOOKS테이블의 bookOriginPrice를 업데이트하고, 
--BOOKS_SELL테이블의 bookSellPrice 값도 변경해주어야 한다. 
...
CALL UPDATE_BOOK(500, 500, 500, @RESULT);
SELECT @RESULT;
